...
class Simulator3:
    def __init__(self, dirichlet, price_matrix, arms_configuration, number_user, distributions, slot_matrix, lambda_):
        ...
        :param number_user: fixed number of users per day
        ...
        count = 0
        self.users = []
        for count in range(self.number_user):
            # Exactly 1/3 of users for each class
            type_ = count % 3
            i = 0
            res_values = []
            for i in range(5):
    ...
    def run(self):
        ...
        for count in range(len(webpages)):
            if useful_users[count].buy(webpages[count], 10):  # Fixed number of items bought
                i = 0
